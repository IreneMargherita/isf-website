    ...
    <>
      ...
      <section className="container-ministry py-16 sm:py-20">
        <div className="mx-auto max-w-3xl text-center">
          ...
          <h2 className="mt-4 text-3xl font-semibold sm:text-4xl">{about.preEvangelism.title}</h2>
          <div className="mx-auto mt-5 flex max-w-2xl flex-col gap-4">
            {about.preEvangelism.body.map((p, i) => (
              <p key={i} className="text-lg leading-relaxed text-ink-600">
                {p}
              </p>
            ))}
        ...
        <ol className="mt-12 grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
          {about.preEvangelism.steps.map((step, i) => (
            <li key={step.title}>
      ...
      <section className="bg-ocean-900 text-white">
        <div className="container-ministry py-16 text-center sm:py-20">
          <div className="mx-auto flex max-w-3xl flex-col gap-5">
            <h2 className="text-3xl font-semibold text-white sm:text-4xl">{about.heart.title}</h2>
            {about.heart.body.map((p, i) => (
              <p key={i} className="text-lg leading-relaxed text-ocean-100/90">
                {p}
              </p>
            ))}
            <blockquote className="mx-auto mt-4 max-w-2xl border-t border-white/20 pt-6 font-display text-xl italic text-ocean-50 sm:text-2xl">
              {about.heart.verse}
            </blockquote>
          </div>
        </div>
      ...
      <CallToAction
        ...
        description="The best way to understand ISF is to share a meal with us. You are always welcome."
